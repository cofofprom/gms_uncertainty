...
def fixed_model_experiment(covar, true_edges, config):
    solver = MHTSolver(config['sig_alpha'], pcorr_pvalues, pcorrcoef)
...
def multiple_models_experiment(density, config, n_jobs=None):
    ...
    with mp.Pool(n_jobs) as pool:
        ...
        for model in models:
            ...
            true_adj = (pcorr != 0.).astype(int) - np.eye(config['N'])
...
def main():
    ...
    exp_name = f'dim{config["N"]}_n{config["n"]}_Ssg{config["S_sg"]}_Sobs{config["S_obs"]}_pcorr'
